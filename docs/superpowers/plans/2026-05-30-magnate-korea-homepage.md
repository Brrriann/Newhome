...
```bash
npm install framer-motion @splinetool/react-spline lucide-react emailjs-com
```
...
@layer base {
  ...
  /* Dot grid pattern — reusable via class */
  .dot-grid {
...
- [ ] **Step 3: Download Pretendard font files**
...
- [ ] **Step 5: Simplify `src/app/page.tsx` to a placeholder**
...
```bash
git add src/app/layout.tsx src/app/globals.css tailwind.config.ts src/app/page.tsx
...
Save as `public/favicon.ico` (convert using any ICO converter) and `public/icon.png`.

- [ ] **Step 3: Commit**

```bash
git add public/og-image.png public/favicon.ico public/icon.png
